...
def create_tables():
    ...
    create_il_jobs_2017_table(conn)
    create_il_geo_xwalk_table(conn)
...
def create_il_jobs_2017_table(conn):
    """
    Create a table for the 2017 IL jobs data
    """
    execute_sql_script(conn, "05_create_il_jobs_2017_table.sql")


def create_il_geo_xwalk_table(conn):
    """
    Create a table for the IL geographic crosswalk data
    """
    execute_sql_script(conn, "06_create_il_geo_xwalk_table.sql")
...
def copy_csv_files(data_files_dict):
    ...
    for name, files in data_files_dict.items():
        ...
        elif name == "il_jobs_2017":
            copy_csv_to_il_jobs_2017_table(conn, csv_file)
        elif name == "il_geo_xwalk":
            copy_csv_to_il_geo_xwalk_table(conn, csv_file)
...
def copy_csv_to_il_jobs_2017_table(conn, csv_file):
    """
    Copy the csv contents of the 2017 IL jobs data into the table
    """
    COPY_IL_JOBS_2017 = "10_copy_il_jobs_2017_to_table.psql"
    copy_expert_psql_script(conn, COPY_IL_JOBS_2017, csv_file)


def copy_csv_to_il_geo_xwalk_table(conn, csv_file):
    """
    Copy the csv contents of the IL geographic crosswalk data into the table
    """
    COPY_IL_GEO_XWALK = "11_copy_il_geo_xwalk_to_table.psql"
    copy_expert_psql_script(conn, COPY_IL_GEO_XWALK, csv_file)
